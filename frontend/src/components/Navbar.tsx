...
export default function Navbar() {
    ...
    const newNavLinks = navlinks.filter(link => link.name !== "Login");
    ...
    return (
        <>
            ...
            >
                <Link to="/">
                    <img className="h-8.5 w-auto" src="/assets/ge.png" alt="logo" width={130} height={34} />
                </Link>

                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    {newNavLinks.map((link: INavLink) => (
                        <NavLink key={link.name} to={link.href} className="hover:text-blue-500 transition">
                            {link.name}
                        </NavLink>
                    ))}
                </div>
